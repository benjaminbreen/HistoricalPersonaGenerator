...
export const buildAnnotationPrompt = (source, options) => {
  ...
  return [
    ...
    '- gender_role must plainly include woman, man, or another historically supported role. Preserve explicit source pronouns and any locked_subject gender; never guess from a name against the source.',
    '- religion_and_ritual starts with a short standard label (for example Anglican, Roman Catholic, Sunni Muslim, Hindu), followed by a colon and no more than one short contextual sentence.',
    '- Name the historically appropriate language stage when known (for example Early Modern English rather than Modern English in seventeenth-century London).',
    '- In clothing_and_possessions, make each garment or object a separate short array item. Do not combine a full outfit into one item.',
